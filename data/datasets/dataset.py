...
import pytorch_lightning.utilities.seed as se
...
class Dataset(object):
    ...
    def download_dataset(self, dataset_dir, dataset_url):
        ...
        raise Exception("Please download the dataset manually.")
...
class ImageDataset_MultiInput(Dataset):

    def __init__(self, train, query, gallery, **kwargs):
        super(ImageDataset_MultiInput, self).__init__(train, query, gallery, **kwargs)
    ...
    def __getitem__(self, index):
        ...
        for i in range(len(img_paths)):
            ...
            exist = Path(img_paths[i]).exists()
            if not exist:
                from PIL import Image
                data = np.zeros((cfg.head_height, cfg.head_width, 3), dtype=np.uint8)
            ...
            if self.transform[i] is not None:
                ...
                #global_logger.warning(f'No transform for the {i + 1}-th image, read and resize')
