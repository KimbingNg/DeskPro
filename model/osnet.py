...
class OSBlock(nn.Module):
    ...
    def forward(self, x):
        identity = x
        x = self.conv1(x)
        x = self.conv3(self.gate(self.conv2a(x)) +
...
class OSNet(nn.Module):
    ...
    def forward(self, v, return_featuremaps=False):
        v = self.featuremaps(v)
...
def init_pretrained_weights(model, key=''):
    ...
    print('Looking for pretrained model from', model_dir)
    try:
    ...
    if not os.path.exists(cached_file):
        print(f'Downloading pretrained weights from {pretrained_urls[key]}...')
